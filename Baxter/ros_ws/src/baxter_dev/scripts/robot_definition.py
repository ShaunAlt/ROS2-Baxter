...
class Robot_Object():
    '''
    Generic Baxter Robot Object
    -
    Contains a collection of generically shared functionality to use across all
    Baxter robot use parts.
...
class Button():
    ...
    def __init__(
            ...
            sub_pos: str,
            comment: str = '',
            change_action: Union[None, Callable[..., Any]] = None
    ) -> None:
        '''
        ...
        - comment : `str`
            - Value for the `Button` object's `comment` attribute.
        - change_action : `Callable[..., Any] | None`
            - Function to override the `change_action` method.
        ...
        self._btn.state_changed.connect(self.change_action)

        # creating the override change-action method
        self._change_action_override_method = change_action

    # ==================================
    # Custom REPR Function of the Button
    def __repr__(self):
        return (
            f'Button(\n\tcomment = {self.comment},\n\t_btn_str = ' \
                + f'{self._btn_str},\n\t_btn = {self._btn},\n\t' \
                + '_change_action_override_method = ' \
                + f'{self._change_action_override_method}\n)'
        )
    
    # =================================
    # Custom STR Function of the Button
    def __str__(self):
        return f'< Button comment={self.comment}, _btn_str={self._btn_str} />'

    # =====================
    # State-Change Listener
    def change_action(self, value: bool) -> None:
        '''
        State-Change Listener
        -
        ...
        # if override method exists, use that
        if self._change_action_override_method is not None:
            self._change_action_override_method(value)
        else:
            rospy.loginfo(
                f'Button Press Read\n\t| - {self}\n\t| - Value: {value}'
            )
